...
def run_python_code(code:str,input_data:str="",timeout:int=8)->str:
    ...
    try:
        ...
        return "Execution Error:\n"+(
            error if error else f"Process exited with code {result.returncode}"
        )
...
Generate exactly 4 specific test scenarios.

Include:
1. Normal case
2. Boundary case
3. Edge case
4. Invalid input case where appropriate
...
Do not write explanations outside the four scenarios."""
...
- Define every function before it is called.
...
- Do not use Markdown.
- Do not use code fences.
- Define every function that is called.
- Define every variable before use.
- Fix NameError.
- Fix SyntaxError.
- Fix TypeError.
- Fix indentation problems.
- Fix logical problems visible from the error.
...
def real_time_tester(state:CrewState):
    ...
    repairs="\n".join(
        repair_history
    ) if repair_history else "No repairs were required."
...
<p>
Generate · Debug · Test · Execute with AI
</p>
